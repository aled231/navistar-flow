...
class FlowRateAppClient:
    ...
    def Automode(self): #This is the function for starting the test and asking at what flow rates to perform the test
        ...
        while True:
            ...
            #self.turbinegpm = self.waterflow
        
            #self.flowlabel = Label(text="Flowrate ="f"{self.turbinegpm:01}")
            #self.flowlabel.grid(row=1, column=0)
            self.root.update()
    ...
    def measure_flow_rate(self):
        ...
        while True:
            ...
            self.turbinegpm = self.waterflow/350
        
            self.flowlabel = Label(text=" Tubrine Flowrate ="f"{round (self.turbinegpm, 2)}")
            self.flowlabel.grid(row=1, column=0)
            self.root.update()

            if channel.voltage > 0.53:
                self.emaggpm = channel.voltage * 3.9
            else:
                self.emaggpm = 0.00

            self.emaglabel = Label(text="EMAG Flowrate ="f"{round (self.emaggpm, 2)}")
            self.emaglabel.grid(row=0, column=0)
            self.root.update()
            

            self.flow_meter1_values.append(self.turbinegpm)
            self.flow_meter2_values.append(self.emaggpm)

            #self.flow_rate_label.configure(text=f"Flow Rate Meter 1: {flow_rate_meter1:.2f} GPM")#, Flow Rate Meter 2: {flow_rate_meter2:.2f} GPM")

            self.flow_rate_history.append(self.flow_meter1)
            self.time_intervals.append(len(self.flow_rate_history) * 2)  # 2-second intervals

        timestamp = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
        filename = f"flowMeterData_{timestamp}.txt"
        self.write_to_file(filename, self.turbinegpm, self.emaggpm)
